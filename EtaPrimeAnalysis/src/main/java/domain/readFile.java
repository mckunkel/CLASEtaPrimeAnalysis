...
public class readFile {
	...
	public static void main(String[] args) {
		...
		RF.setNinputFile(1); // --> Set the number of hipo-files you want to
								// analyse
		RF.setReaders("/Users/daniellersch/Desktop/CLAS/EtaPrimeDilepton", "fullEtaPrimeDilepton_"); // -->Define
																										// the
																										// directoy,
																										// where
																										// the
		...
		// Look at missing mass deduced from beam, target, proton and one
		// electron
		...
		for (int iF = 0; iF < RF.getNinputFile(); iF++) {
			...
			for (int ev = 0; ev < Nevents[iF]; ev++) {
				...
				// ja... it seems that this is the current way to add particle
				// vectors
				...
				// The event-filter here is based on the current
				// PID-implementation...
				// We can do better than that...
				// The filter also sets the particle 4momentum-vectors....
				// Also you can define whether you are looking at MC or
				// REC-events...
				if (RF.Filter("REC") == 1) {
					missing_rec_particle_comb1.combine(recParticle[2], -1);
					missing_rec_particle_comb1.combine(recParticle[3], -1);
					missing_mass_rec_comb1 = 0.001 * missing_rec_particle_comb1.mass(); // -->
																						// I
																						// am
																						// not
																						// sure
																						...
																						// scaling
																						// factor
					// I have the hunch, that either I am doing something wrong,
					// or that components of the 4Vector might need to be scaled
					// because of the unit-issues
	...
	public void loadReaction(String[] Reaction) {
		...
		recParticle = new Particle[NdecayParticles + 2]; // Because we have beam
															// and target --> 2
															// extra particles
		genParticle = new Particle[NdecayParticles + 2]; // Because we have beam
															// and target --> 2
															// extra particles
				...
				while (current_particle < NdecayParticles) {
					...
					for (int t = 0; t < Nparticles; t++) {
						...
						if (currentPid == requestedID[current_particle] && matchedID == 0 && usedEntry[t] == 0) {
							...
							px = currentBank.getFloat("px", t); // I know having
																// a double as a
																// float... but
																// the
																// setVector-methoid
							...
							vx = currentBank.getFloat("vx", t); // I know having
																// a double as a
																// float... but
																// the
																// setVector-methoid
