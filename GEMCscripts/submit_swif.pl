...
for $a ( 0 .. $#torusValue ) {
	for $b ( 0 .. $#solenoidValue ) {
		...
		my $torusSol_dir =
		  "Torus" . $torusValue[$a] . "Sol" . $solenoidValue[$b];
		my $workflow =
		    "-workflow full_sim_tor"
		  . $torusValue[$a] . "sol"
		  . $solenoidValue[$b];
		...
		#Options to input into Gemc
		my $setTorusString =
		  "value=\"clas12-torus-big, " . $torusValue[$a] . "\"";
		my $setTorus =
		  "perl -p -i -e 's/$torusString/$setTorusString/g' clas12.gcard";

		my $setSolenoidtString =
		  "value=\"clas12-solenoid, " . $solenoidValue[$b] . "\"";
		my $setSolenoid =
"perl -p -i -e 's/$solenoidString/$setSolenoidtString/g' clas12.gcard";
		...
		while ( $iJob < $nJobs ) {
			...
			my $input_2 =
"-input fullEtaPrimeDilepton.lund $lundInput_dir/AllDalitz_$iJob.lund";

			#check to see in gemc file already exists
			my $gemc_out =
			    "$gemcOutput_dir/FullDilepton_Tor"
			  . $torusValue[$a] . "Sol"
			  . $solenoidValue[$b] . "_"
			  . $iJob . ".ev";
