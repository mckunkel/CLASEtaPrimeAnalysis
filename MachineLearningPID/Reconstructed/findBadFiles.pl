...
for my $p (0 .. $#particleValue){#interate through particles
  for my $a (0 .. $#torusValue){#interate through torus
    for my $b (0 .. $#solenoidValue){#interate through solenoid
      ...
      my $gemcOutput_dir = "$outDir/$torusSol_dir";
      #find average file size from all files in $gemcOutput_dir
      while($iJob <= $nJobs){
        my $gemc_out = "$gemcOutput_dir/out_".$particleValue[$p]."_Tor".$torusValue[$a]."Sol".$solenoidValue[$b]."_".$iJob.".hipo";
        if(-e $gemc_out){
          $doneJobs++;
