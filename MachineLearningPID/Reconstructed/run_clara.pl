...
my $clas12Dir = "/work/clas12/mkunkel/EtaPrimeDilepton/CLASEtaPrimeAnalysis/MachineLearningPID/Reconstructed";
my $userName = "mkunkel";
...
my $nJobs = 400;	# total number of jobs 469
...
my $session = 0;
for my $p (0 .. $#particleValue){#interate through particle
  for $a (0 .. $#torusValue){#interate through torus
    for $b (0 .. $#solenoidValue){#interate through solenoid
      ...
      my $claraList = "fileList".$particleValue[$p].$torusValue[$a]."Sol".$solenoidValue[$b];
      ...
      my $iJob = 1;
      while($iJob <= $nJobs){
        my $decodedData = $particleValue[$p]."_Tor".$torusValue[$a]."Sol".$solenoidValue[$b]."_".$iJob.".hipo";
        my $decoded_out = "$decoded_dir/$decodedData";
        my $reconData = "out_".$particleValue[$p]."_Tor".$torusValue[$a]."Sol".$solenoidValue[$b]."_".$iJob.".hipo";
